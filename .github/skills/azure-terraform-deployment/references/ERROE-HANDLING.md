...
- **Severity:** Error
- **Source:** terraform/azurerm (via Azure CLI)
- **Message:** Not logged in to Azure CLI (Terraform cannot obtain Azure credentials)
- **Remediation:** Run `az login` (and `az account set --subscription "<id>"` if needed), then re-run.
- **Documentation:** https://learn.microsoft.com/en-us/cli/azure/authenticate-azure-cli
```
